...
at::Tensor nms_cuda_forward(const at::Tensor boxes, float nms_overlap_thresh) {
    ...
    int col_blocks = CEIL_DIV(boxes_num, threadsPerBlock);

    scalar_t* boxes_dev = boxes_sorted.data_ptr<scalar_t>();

    auto mask = at::empty({boxes_num * col_blocks},
                        boxes.options().dtype(at::kLong));
    auto mask_dev = reinterpret_cast<unsigned long long*>(mask.data_ptr<int64_t>());
    ...
    cudaMemcpyAsync(mask_host.data(),mask_dev,
